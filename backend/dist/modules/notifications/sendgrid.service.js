...
function sendgridConfig(configService) {
    const cfg = configService || new config_1.ConfigService();
    return {
        apiKey: cfg.get('SENDGRID_API_KEY'),
        fromEmail: cfg.get('SENDGRID_FROM_EMAIL') || 'noreply@urbanhelp.com.au',
    };
}
let SendGridService = class SendGridService {
    ...
    getFromEmail() {
        return this.configService.get('SENDGRID_FROM_EMAIL') || 'noreply@urbanhelp.com.au';
    }
    ...
    `;
        try {
            await mail_1.default.send({ to: businessEmail, from: config.fromEmail, subject: 'Urban Help - New Booking Request', html: htmlContent });
        }
        catch (error) {
    ...
    `;
        try {
            await mail_1.default.send({ to: customerEmail, from: config.fromEmail, subject: 'Urban Help - Payment Request', html: htmlContent });
        }
        catch (error) {
    ...
    `;
        try {
            await mail_1.default.send({ to: email, from: config.fromEmail, subject: 'Urban Help - Password Reset', html: htmlContent });
        }
        catch (error) {
    ...
    `;
        try {
            await mail_1.default.send({ to: email, from: config.fromEmail, subject: 'Urban Help - Password Reset Confirmation', html: htmlContent });
        }
        catch (error) {
    ...
    async sendPaymentFailedEmail(email, businessName, reason) {
        const config = sendgridConfig();
        const htmlContent = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
    ...
    `;
        try {
            await mail_1.default.send({ to: email, from: config.fromEmail, subject: 'Urban Help - Payment Failed', html: htmlContent });
        }
        catch (error) {
    ...
    `;
        try {
            await mail_1.default.send({ to: email, from: config.fromEmail, subject: 'Urban Help - Account Temporarily Locked', html: htmlContent });
        }
        catch (error) {
    ...
    async sendPayoutProcessedEmail(businessEmail, businessName, amount) {
        const config = sendgridConfig();
        ...
        try {
            await mail_1.default.send({ to: businessEmail, from: config.fromEmail, subject: "Urban Help - Payout Processed", html: htmlContent });
        }
        catch (error) {
    ...
    async sendEmail(to, subject, html) {
        const config = sendgridConfig();
        try {
            await mail_1.default.send({ to, from: config.fromEmail, subject, html });
        }
        catch (error) {
